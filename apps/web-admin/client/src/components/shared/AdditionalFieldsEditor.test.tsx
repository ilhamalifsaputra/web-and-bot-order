...
describe("AdditionalFieldsEditor", () => {
  it("renders no field rows and an Add Field button when value is empty", () => {
    ...
    expect(screen.queryAllByPlaceholderText(/e\.g\. game_id/i)).toHaveLength(0);
    expect(screen.getByRole("button", { name: /add field/i })).toBeInTheDocument();
  });

  it("renders one row per draft, showing the current key/labels/placeholder", () => {
  ...
  it("typing in a row's key input reports the updated draft via onChange", () => {
    const onChange = vi.fn();
    const drafts: AdditionalFieldDraft[] = [{ ...emptyFieldDraft(), key: "" }];
    render(<AdditionalFieldsEditor value={drafts} onChange={onChange} />);
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. game_id/i), { target: { value: "server_id" } });
    expect(onChange).toHaveBeenCalledWith([{ ...emptyFieldDraft(), key: "server_id" }]);
  });
