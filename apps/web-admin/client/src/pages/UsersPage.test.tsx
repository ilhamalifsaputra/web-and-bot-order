...
// Dedi: joined within the last 7 days, not banned, fewer than 2 delivered
// orders — drives the New Customer badge (takes precedence over Returning).
const USER_DEDI = {
  ...USER_ANDI,
  id: 4,
  username: "dedi",
 ...
 *  `onPost` lets ban/unban tests observe the posted body per URL. */
...
describe("UsersPage", () => {
  ...
  it("shows the Banned badge for a banned row, New Customer for a recently-joined row, and Returning otherwise", async () => {
    mockFetchRouter({
      users: {
        users: [USER_ANDI, USER_BUDI, USER_DEDI],
        total: 3,
        page: 1,
        pageSize: 20,
        hasNext: false,
    ...
    const dediRow = screen.getByText("Dedi Wijaya").closest("tr")!;

    // Andi: not banned, not recently joined, >=2 delivered orders — Returning.
    expect(within(andiRow).getByText("Returning")).toBeInTheDocument();
    expect(within(andiRow).queryByText("Banned")).not.toBeInTheDocument();

    // Budi: banned takes precedence over any other status badge.
    expect(within(budiRow).getByText("Banned")).toBeInTheDocument();
    expect(within(budiRow).queryByText("New Customer")).not.toBeInTheDocument();

    // Dedi: joined within the last 7 days, not banned — New Customer badge,
    // even though deliveredOrders is 0 (would otherwise show nothing).
    expect(within(dediRow).getByText("New Customer")).toBeInTheDocument();
    expect(within(dediRow).queryByText("Banned")).not.toBeInTheDocument();
  });
