...
export default async function stockRoutes(app: FastifyInstance): Promise<void> {
  // ---- Stock list + stock product: migrated to React SPA (api/stock.ts) ----
  // GET /stock  →  served by SPA shell (StockPage)
  // GET /stock/:productId  →  served by SPA shell (StockProductPage)
