...
*   **Upgrading a guest row closes the recovery path.** `/account/settings` is reachable by a guest (only the account-menu *link* is hidden), and the credentials endpoint skips the current-password re-auth while `passwordHash` is null — so a guest can turn their synthetic row into a real, password-protected account. `setLoginCredentials` therefore sets `isGuest: false` and nulls `guestEmail` the moment it writes a `passwordHash` (the password-reset endpoint lands there too). Without that, the old guest address plus an order code would still mint a session on the now-protected account and rotate the owner's live session out.
...
*   **Buyer notifications.** Guests have no `telegramId`, so the outbox's buyer DMs are skipped for them (they already guard on a null id). Admin alerts are unaffected. Their channel-post label carries a hint derived from the last two digits of the guest's `User.id` — nothing from the email ever reaches the public channel or the outbox row.
*   **The order-code email.** A guest's *recovery code* **is** emailed, when the shop has SMTP configured: `POST /api/v1/checkout` sends the order code plus a link to the order and a link to `/track` (`sendGuestOrderCodeEmail`, `apps/storefront/src/routes/api.ts`), because the code otherwise existed only on one screen and losing the tab lost the order for good. What that email may carry is strictly bounded: **the code and links, never the delivered content or credentials** — email is unencrypted and lives in an inbox forever, so guests still read what they bought on the web and nothing about it is ever mailed. Signed-in buyers get no such mail (they have an account and an orders page). Sending is best-effort and can never fail an order: no SMTP configured means the feature is simply off, a throwing or slow `sendMail` is caught and logged (never with the address), and the `201` answers a guest with `email_sent: boolean` so the SPA promises an inbox only when one was actually written to — that flag is carried across the post-checkout page load to the pay page, which is where the confirmation line is shown (`client/src/lib/orderCodeEmailed.ts`).
